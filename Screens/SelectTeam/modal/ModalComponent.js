          ...
          >
            <Text style={{ textAlign: "center" }}>
              <Text style={{ fontSize: FONTSIZE.large }}>
                Are you sure you want to entry{" "}
              </Text>
              {selectedTeams?.map((data, index) => (
                <Text key={index}>
                  {index + 1 === selectedTeams.length &&
                  selectedTeams.length >= 2 ? (
                    <Text> and </Text>
                  ) : null}
                  <Text
                    style={{ fontWeight: "bold", fontSize: FONTSIZE.detail }}
                  >
                    {data.league_participant.team_profile.name}
                  </Text>
                  {index === selectedTeams.length - 2 ||
                  index + 1 === selectedTeams.length ? null : (
                    <Text>, </Text>
                  )}
                </Text>
              ))}
