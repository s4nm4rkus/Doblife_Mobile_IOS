...
import { fetchMyTeamRosters, fetchTeamRosters } from '../../../api/teamRosterApi';
...
const profileImageUrl = 'https://images.pexels.com/photos/2834914/pexels-photo-2834914.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1';
      ...
      <View style={styles.cardContainer}>
          ...
          <ScrollView contentContainerStyle={styles.scrollViewContent} nestedScrollEnabled={true}>
            {
              teamRosters.length == 0 ? (
                <View style={styles.rowContainer}>
                  <Text style={styles.noTeamsText}>No available teams on the list</Text>
                </View>
              ) : (
                teamRosters?.map((item, index) => (
                  <View style={styles.rowContainer} onStartShouldSetResponder={() => true} key={index}>
                    <TouchableOpacity
                      key={item}
                    ...
                    >
                      {
                        item.league_participant.team_profile.default_team_profile_pic ? (
                          <Image source={{ uri: item.league_participant.team_profile.default_team_profile_pic.image }} resizeMode="contain" style={styles.teamLogo} />
                        ) : (
                          <Image source={require('../../../assets/teamPlaceholders/team-placeholder-04.png')} resizeMode="contain" style={styles.teamLogo} />
                        )
                      }
                      <Text style={styles.teamName}>{item.league_participant.team_profile.name}</Text>
                    </TouchableOpacity>
                  </View>
                ))
              )
        ...
        <View style={styles.paginationContainer}>
          <View style={styles.paginationWrapper}>
            ...
            <ScrollView contentContainerStyle={styles.scrollViewButtonContainer} horizontal={true} showsHorizontalScrollIndicator={false}>
              {(
